...
function getNslMarkers(data) {
  ...
  $.each(data, function(i, value) {
    ...
    nslMarker.V_P_NAME = value.V_P_NAME; //所属省
    nslMarker.V_C_NAME = value.V_C_NAME; //所属市
    nslMarker.V_CT_NAME = value.V_CT_NAME; //所属县
    nslMarker.V_T_NAME = value.V_T_NAME; //所属乡镇
    nslMarker.V_DF_NAME = value.V_DF_NAME; //泥石流沟名称
    nslMarker.V_DF_CODE = value.V_DF_CODE; //泥石流沟代码
    nslMarker.V_AC_CODE = value.V_AC_CODE; //行政村代码
    nslMarker.V_M_CODE = value.V_M_CODE; //中小河流代码
    nslMarker.V_R_AREA = value.V_R_AREA; //流域面积
    nslMarker.V_G_CROSECTION = value.V_G_CROSECTION; //沟槽横截面
    nslMarker.V_TD_lONSLOPE = value.V_TD_lONSLOPE; //主沟纵坡

    nslMarker.V_DF_SITE_LOR = value.V_DF_SITE_LOR; //泥石流沟位于汇入河的位置
    nslMarker.V_DF_DIS_MOUTORIVER = value.V_DF_DIS_MOUTORIVER; //沟口至主河道的距离
...
function nslWin() {
  ...
  var contentHTML =
    ...
    this.V_TD_lONSLOPE +
    "</td></tr>" +
    "<tr><td>泥石流沟位于汇入河的位置：</td><td>" +
    this.V_DF_SITE_LOR +
    "</td></tr>" +
    "<tr><td>沟口至主河道的距离：</td><td>" +
    this.V_DF_DIS_MOUTORIVER +
    "</td></tr>" +
    "<tr><td colspan=\"2\" style='text-align:right;'>" +
    "<a href='../nishiliudetails.html?vmcode=" +
    this.V_DF_CODE +
    "&area=" +
 ...
 * @param data
 */
function getNslYhdMarkers(data) {
  GoclearMarkers();
  //创建标记图层
...
function getNslYjdMarkers(data, hsleval, shixiao) {
  GoclearMarkers();
...
function getNslXzcMarkers(data) {
  //创建标记图层
  ...
  $.each(data, function(i, dataObj) {
    ...
    var V_AC_NAME =
      dataObj.V_AC_NAME == null ? dataObj.V_DF_NAME : dataObj.V_AC_NAME; //乡镇名称
    var V06001_DF = dataObj.V06001_DF; //x
    var V05001_DF = dataObj.V05001_DF; //y

    var V_DF_CODE = dataObj.V_DF_CODE; //泥石流沟编码
    ...
    var V_DAM_BUILDING = dataObj.V_DAM_BUILDING; //损坏房屋
    ...
    var marker = new SuperMap.Marker(
      new SuperMap.LonLat(V06001_DF, V05001_DF).transform(
        "EPSG:4326",
        "EPSG:900913"
      ),
      icon
    ...
    marker.events.on({
      click: openNslDetailsXzcView,
      scope: marker
    });

    marker.V_AC_CODE = V_AC_CODE;
    marker.V_AC_NAME = V_AC_NAME;
    marker.V_DF_CODE = V_DF_CODE;
    marker.V06001_DF = V06001_DF;
    marker.V05001_DF = V05001_DF;
    ...
    marker.V_DAM_BUILDING = V_DAM_BUILDING;
...
function openNslDetailsXzcView() {
  clearPopup();
  var marker = this;
  var V_AC_CODE = marker.V_AC_CODE; //乡镇代码
  var V_AC_NAME = marker.V_AC_NAME; //乡镇名称
  var V_DF_CODE = marker.V_DF_CODE; //流域编码
  var V06001_DF = marker.V06001_DF; //x
  var V05001_DF = marker.V05001_DF; //y
  ...
  var V_DAM_BUILDING = marker.V_DAM_BUILDING;
  ...
  contentHTML +=
    "<tr><td>≥损坏房屋:</td><td>" +
    (V_DAM_BUILDING == "" || V_DAM_BUILDING == undefined
      ? "-"
      : V_DAM_BUILDING + "&nbsp;人") +
    "</td></tr>";
  contentHTML +=
...
function getNslYjdDetails(vmcode) {
  //预警点位置
 ...
 */
function getNslYhdDetails(area_code) {
  $.ajax({
    async: true,
    cache: true,
    type: "POST",
    dataType: "json",
    url: "../getNslYhdDetails.do", //中小河流域对应的镇信息
    timeout: 60000,
    data: {
      vmcode: area_code
    },
