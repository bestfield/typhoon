...
SuperMap.Layer.Tianditu = SuperMap.Class(SuperMap.CanvasLayer,{
	 ...
	 * {String} 图片url.
	 */
	url : "http://t${num}.tianditu.com/DataServer?T=${type}_${proj}&x=${x}&y=${y}&l=${z}&tk=762ddf2edececee9e99ac2c07f4e29e0",
	...
	clone : function(obj) {
		var me = this;
		if (obj == null) {
			obj = new SuperMap.Layer.Tianditu(me.name, me.url,
					me.params, me.getOptions());
		}

		obj = SuperMap.CanvasLayer.prototype.clone.apply(me,
				[ obj ]);
		obj._timeoutId = null;
	...
	getTileUrl : function(xyz) {
		...
		if (this.isLabel) {
			if (this.layerType == "vec")
				lt = "cva";
			if (this.layerType == "img")
				lt = "cia";
			if (this.layerType == "ter")
				lt = "cta";
		}
	 ...
	 */
	setMap : function(map) {
		SuperMap.CanvasLayer.prototype.setMap.apply(this,
				[ map ]);
		var proCode = null;
	...
	setTiandituParam : function(projection) {
		...
		if (projection == "EPSG:4326") {
			...
			this.units = "degree";
			this.projection = new SuperMap.Projection(
					"EPSG:4326");

			this.maxExtent = new SuperMap.Bounds(minX, minY,
					maxX, maxY);
			...
			var resolutions = [];
			for ( var i = resStart; i < resLen; i++) {
				resolutions.push(156543.0339 / 2 / Math.pow(2,
						i));
			}
			//this.numZoomLevels = 18;
			...
			this.maxExtent = new SuperMap.Bounds(minX, minY,maxX, maxY);
			this.tileOrigin = new SuperMap.LonLat(minX, maxY);
			this.resolutions = resolutions;
		}
	},
